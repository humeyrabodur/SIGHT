...
//3=left
//4=turn back
//5= terminate the search and stay where you are 


///////IF THE CARD IS READ BEFORE THE MOVE FORWARD, THE RFID MAY NOT BE READ
...
while(search_continue){
  ...
  if (Serial.available() > 0) { // Check if data is available to read
    ...
    uint8_t move_indicator = Serial.read(); // Read the incoming byte
    if (move_indicator != oldData){
    Serial.print("Received: ");

    Serial.println("An RFID card has been read");
    if(move_indicator==0){
      Serial.println("Mu must not move");
    ...
    if(move_indicator==5){
      ...
      Serial.println("Terminate the search");
      search_continue=0;
      break;
    }

    oldData = move_indicator;
